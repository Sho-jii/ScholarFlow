...
export const TextStaggerHover = ({
  ...
  const [internalMouseIn, setInternalMouseIn] = React.useState<boolean>(false);
  const isMouseIn = isHovered !== undefined ? isHovered : internalMouseIn;
  ...
  return (
    <TextStaggerHoverContext.Provider value={{ isMouseIn }}>
      <Component
        ...
        {...props}
        onMouseEnter={() => setInternalMouseIn(true)}
        onMouseLeave={() => setInternalMouseIn(false)}
      >
        {children}
      </Component>
...
export const TextStaggerHoverHidden = ({
  ...
  const { isMouseIn } = useTextStaggerHoverContext();
  return (
    <span className={cn('inline-block absolute left-0 top-0 text-nowrap pointer-events-none', className)}>
      {characters.map((char, index) => {
        const staggerDelay = setStaggerDirection({
          direction: staggerDirection,
          totalItems: characterCount,
